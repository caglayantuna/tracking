...
# ========== CONFIG ==========
RFDETR_WEIGHTS = os.path.expanduser(
    "/pasteur/helix/users/ctuna/my_models/rfdetr_xlarge.pth"
)
...
def draw_overlay(frame_bgr: np.ndarray,
    ...
    for obj_id in obj_ids:
        ...
        color_layer[binary > 0] = _OBJ_COLOR
        contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(overlay, contours, -1, _OBJ_COLOR, 1)
...
def main(video_path: str):
    ...
    OUTPUT_DIR  = os.path.expanduser(f"./output_rfdetr_track_xlarge/{video_stem}/")
    ...
    tracked_video   = os.path.join(OUTPUT_DIR, f"{video_stem}_tracked_rfdetr_track.mp4")

    model   = RFDETRSegXLarge(pretrain_weights=RFDETR_WEIGHTS)
    ...
    while True:
        ...
        if frame_idx % 100 == 0:
            ...
            print(f"[{frame_idx:>6}/{total_frames}] {elapsed:6.1f}s  {throughput:.1f} fps")
        ...
        detections = model.predict(frame_rgb, threshold=DETECTION_THRESHOLD)
        ...
        if detections.mask is not None and detections.tracker_id is not None:
            for tid, mask in zip(detections.tracker_id, detections.mask):
                m = mask.astype(np.uint8)
                if m.shape != (h, w):
                    m = cv2.resize(m.astype(np.float32), (w, h), interpolation=cv2.INTER_LINEAR)
                labeled_mask[m > 0.5] = int(tid)
        ...
        # ── Overlay ───────────────────────────────────────────────────
        overlay = draw_overlay(frame_bgr, labeled_mask, frame_idx)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Track larvae with RF-DETR segmentation + ByteTrack."
    )
    parser.add_argument("video_path", help="Path to input video")
    parser.add_argument(
        "--weights",
        default=RFDETR_WEIGHTS,
        help="Path to RF-DETR checkpoint (default: %(default)s)",
    )
    parser.add_argument(
        "--threshold",
        type=float,
