"""
Robust larva tracker replacing ByteTrack with a custom tracker designed for
top-view larva videos where:
...
Cluster variant: uses the shared RF-DETR xlarge weights under
/pasteur/helix/users/ctuna/my_models and imports larva_head_tail locally.
"""
...
def main(video_path: str):
    ...
    OUTPUT_DIR  = os.path.expanduser(f"./output_rfdetr_track_robust/{video_stem}/")
    ...
    tracked_video   = os.path.join(OUTPUT_DIR, f"{video_stem}_tracked_robust.mp4")
    ...
    while True:
        ...
        # Extract masks from supervision Detections
        det_masks: List[np.ndarray] = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Track larvae with RF-DETR segmentation + robust custom tracker."
    )
